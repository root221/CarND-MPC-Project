...
size_t a_start = epsi_start + N;
size_t delta_start = a_start + N - 1;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    fg[0] = 0;
    for(int t = 0; t < N; t++){
      fg[0] += 1.1 * CppAD::pow(vars[cte_start + t],2);
      fg[0] += CppAD::pow(vars[epsi_start + t],2);
      fg[0] += CppAD::pow(vars[v_start + t] - 15, 2); // ??
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int i = a_start; i < delta_start; i++){
    vars_lowerbound[i] = -1;
    vars_upperbound[i] = 1;
  }
  ...
  return {solution.x[a_start], solution.x[delta_start]};
}
